"""Hyresbostad.se scraper - Swedish rental apartments (JavaScript rendered)."""
...
class HyresbostandScraper(PlaywrightBaseScraper):
    """Scraper for hyresbostad.se Swedish rental listings.

    Hyresbostad is a Swedish rental apartment listings site.
    The site requires JavaScript rendering.

    URL patterns:
    - Search: https://www.hyresbostad.se/lediga-lagenheter/stockholm
    - Listing: https://www.hyresbostad.se/lagenhet/{id}
    """

    site_name = "hyresbostad"
    ...
    def get_search_url(self) -> str:
        ...
        return f"{self.base_url}/lediga-lagenheter/stockholm"
    ...
    def get_listing_urls(self, page: Page) -> list[str]:
        ...
        try:
            ...
            # TODO: Adjust selector based on actual Hyresbostad HTML structure
            for link in soup.find_all("a", href=True):
                href = link.get("href", "")
                if "/lagenhet/" in href or "/bostad/" in href:
                    listing_match = re.search(r"/(?:lagenhet|bostad)/([^/]+)", href)
                    if listing_match:
    ...
    def parse_listing_page(self, html: str, url: str) -> dict:
        """Parse a Hyresbostad listing page and extract data."""
        soup = BeautifulSoup(html, "lxml")
        data = {}
