"""Bovision.se scraper - Swedish property portal with rentals (JavaScript rendered)."""
...
class BovisionScraper(PlaywrightBaseScraper):
    """Scraper for bovision.se Swedish rental listings.

    Bovision is a Swedish property portal with a rental section.
    The site requires JavaScript rendering.

    URL patterns:
    - Search: https://www.bovision.se/hyra/lagenhet/stockholms-lan
    - Listing: https://www.bovision.se/hyra/lagenhet/{area}/{id}
    """
    ...
    def get_search_url(self) -> str:
        ...
        base = f"{self.base_url}/hyra/lagenhet/stockholms-lan"
        params = []

        if self.min_price:
            params.append(f"minMonthlyRent={self.min_price}")
        if self.max_price:
            params.append(f"maxMonthlyRent={self.max_price}")
    ...
    def get_listing_urls(self, page: Page) -> list[str]:
        ...
        try:
            ...
            # TODO: Adjust selector based on actual Bovision HTML structure
            for link in soup.find_all("a", href=True):
                href = link.get("href", "")
                if "/hyra/" in href and "/lagenhet/" in href:
                    # Match pattern /hyra/lagenhet/{area}/{id}
                    if href.count("/") >= 4:
    ...
    def parse_listing_page(self, html: str, url: str) -> dict:
        """Parse a Bovision listing page and extract data."""
        soup = BeautifulSoup(html, "lxml")
        data = {}
