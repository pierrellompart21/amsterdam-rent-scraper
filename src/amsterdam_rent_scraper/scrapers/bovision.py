"""Bovision.se scraper - Swedish property portal (JavaScript rendered)."""
...
class BovisionScraper(PlaywrightBaseScraper):
    ...
    Bovision is a property portal that lists both rentals and sales.
    The site is JavaScript-heavy. Prices are in SEK.

    URL pattern: /hyra/[area]/[listing-id]
    """
    ...
    def get_search_url(self) -> str:
        ...
        base_search = f"{self.base_url}/hyra/stockholm"
        params = []
        if self.min_price:
    ...
    def get_listing_urls(self, page: Page) -> list[str]:
        ...
        try:
            ...
            for _ in range(10):
                ...
                # TODO: Update selector based on actual Bovision HTML structure
                links = soup.select('a[href*="/hyra/"]')
            ...
            # Find listing links
            # TODO: Update selector based on actual Bovision HTML structure
            for link in soup.select('a[href*="/hyra/"]'):
                href = link.get("href", "")
                # Exclude search/category pages, look for individual listings
                if "/hyra/" in href and href.count("/") > 2:
