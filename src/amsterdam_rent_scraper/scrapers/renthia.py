...
class RenthiaScraper(PlaywrightBaseScraper):
    ...
    URL patterns:
    - Search: https://renthia.com/en/for-rent/stockholm
    - Listing: https://renthia.com/en/for-rent/{area}/{id}
    """
    ...
    def _new_page(self) -> Page:
        """Create a new page with English locale settings (international-friendly site)."""
        ...
        context = browser.new_context(
            ...
            locale="en-SE",  # English in Sweden
        )
    ...
    def get_search_url(self) -> str:
        ...
        base = f"{self.base_url}/en/for-rent/stockholm"
        params = []
    ...
    def get_listing_urls(self, page: Page) -> list[str]:
        ...
        try:
            ...
            # TODO: Adjust selector based on actual Renthia HTML structure
            for link in soup.find_all("a", href=True):
                href = link.get("href", "")
                # Match listing URLs like /en/for-rent/{area}/{id}
                if "/for-rent/" in href and href.count("/") >= 4:
                    url_parts = href.rstrip("/").split("/")
                    if len(url_parts) >= 5:
                        listing_id = url_parts[-1]
                        if listing_id and listing_id not in seen_ids and not listing_id.startswith("stockholm"):
    ...
    def parse_listing_page(self, html: str, url: str) -> dict:
        ...
        # Price - Renthia uses both SEK and EUR formats
        price_patterns = [
            r"(\d[\d\s]*)\s*SEK\s*/\s*month",
            r"(\d[\d\s]*)\s*kr\s*/\s*m(?:ån|ånad|onth)",
            r"rent[:\s]*(\d[\d\s]*)\s*(?:SEK|kr)",
            r"(\d[\d\s,\.]+)\s*EUR\s*/\s*month",
        ]
