...
class RenthiaScraper(PlaywrightBaseScraper):
    ...
    Renthia is an international-friendly rental platform for Sweden.
    ...
    def get_search_url(self) -> str:
        ...
        return (
            f"{self.base_url}/sv/hyr/stockholm"
        ...
        try:
            ...
            for scroll_attempt in range(10):
                ...
                links = [l for l in soup.find_all("a", href=True)
                        if "/rental/" in l.get("href", "") or "/bostad/" in l.get("href", "")]
            ...
            for link in soup.find_all("a", href=True):
                href = link.get("href", "")
                if "/rental/" in href or "/bostad/" in href:
    ...
    def parse_listing_page(self, html: str, url: str) -> dict:
        ...
        # Price - Swedish format (SEK) or EUR
        price_patterns = [
            r"(\d[\d\s]*)\s*kr\s*/\s*(?:mån|månad|month)",
            r"hyra[:\s]*(\d[\d\s]*)\s*kr",
            r"(\d[\d\s]*)\s*SEK",
            r"(\d[\d\s]*)\s*€\s*/\s*(?:mån|månad|month)",
        ]
