"""BostadsZonen.se scraper - Swedish national housing portal."""
...
class BostadszonenScraper(PlaywrightBaseScraper):
    """Scraper for bostadszonen.se rental listings.

    BostadsZonen is a Swedish national housing portal with rental listings
    from all over Sweden, with strong coverage in Stockholm, Göteborg, and Malmö.

    URL patterns:
    - Search: https://bostadszonen.se/sok?locale=en&city=stockholm
    - Listing: /bostad/{id}
    """

    site_name = "bostadszonen"
    ...
    def get_search_url(self, page_num: int = 1) -> str:
        ...
        if self.min_price:
            params["rent_min"] = int(self.min_price)
        if self.max_price:
            params["rent_max"] = int(self.max_price)
        if page_num > 1:
    ...
    def get_listing_urls(self, page: Page) -> list[str]:
        ...
        while current_page <= max_pages:
            try:
                ...
                # Find listing links
                listing_links = soup.find_all("a", href=re.compile(r"/bostad/|/apartment/|/listing/"))

                if not listing_links:
                    # Try finding any links with numeric IDs
                    listing_links = soup.find_all("a", href=re.compile(r"/\d{5,}"))
                ...
                for link in listing_links:
                    ...
                    if "/sok" in href or "?" in href:
                        continue
                    # Extract ID
                    id_match = re.search(r"/(\d+)/?$|/bostad/([^/]+)", href)
                    if id_match:
    ...
    def parse_listing_page(self, html: str, url: str) -> dict:
        """Parse a BostadsZonen listing page and extract data."""
        soup = BeautifulSoup(html, "lxml")
        data = {}
        ...
        if addr_tag:
            ...
            addr_match = re.search(r"([A-ZÄÖÅ][a-zäöå]+(?:gatan|vägen|gata|väg|gränd)\s*\d*)", full_text)
            if addr_match:
        ...
        if re.search(r"\bhiss|elevator|lift\b", full_text, re.IGNORECASE):
            data["has_elevator"] = True
        if re.search(r"\bmöblerad|furnished\b", full_text, re.IGNORECASE):
            data["furnished"] = "Furnished"
        if re.search(r"\bparkering|parking|garage\b", full_text, re.IGNORECASE):
            data["has_parking"] = True

        # Available date
        if re.search(r"omgående|immediately|available\s*now", full_text, re.IGNORECASE):
