"""Bostad.stockholm.se scraper - Official Stockholm housing queue (Bostadsförmedlingen)."""
...
class BostadStockholmScraper(PlaywrightBaseScraper):
    """Scraper for bostad.stockholm.se official housing listings.

    Bostadsförmedlingen is Stockholm's official rental housing agency.
    Apartments are allocated via a queue system, but listings are publicly visible.

    URL patterns:
    - Search: https://bostad.stockholm.se/lista/
    - Listing: /bostad/{id}
    """

    site_name = "bostad_stockholm"
    ...
    def get_search_url(self, page_num: int = 1) -> str:
        ...
        base = f"{self.base_url}/lista/"
        params = {}
        if self.min_price:
            params["hyra_fran"] = int(self.min_price)
        if self.max_price:
            params["hyra_till"] = int(self.max_price)
        if page_num > 1:
            params["sida"] = page_num
        if params:
    ...
    def get_listing_urls(self, page: Page) -> list[str]:
        ...
        while current_page <= max_pages:
            try:
                ...
                # Find listing links - pattern: /bostad/{id}
                listing_links = soup.find_all("a", href=re.compile(r"/bostad/\d+"))
                ...
                for link in listing_links:
                    href = link.get("href", "")
                    id_match = re.search(r"/bostad/(\d+)|/(\d+)/?$", href)
                    if id_match:
                ...
                # Check for next page
                next_link = soup.find("a", href=re.compile(rf"sida={current_page + 1}"))
                next_btn = soup.find("a", class_=re.compile(r"next|nasta"))
    ...
    def parse_listing_page(self, html: str, url: str) -> dict:
        """Parse a Bostad Stockholm listing page and extract data."""
        soup = BeautifulSoup(html, "lxml")
        data = {}
        ...
        price_patterns = [
            r"(\d{1,3}(?:[\s\u00a0]\d{3})*)\s*kr(?:/månad|/mån)?",
            r"(?:Hyra|Månads?hyra)[:\s]*(\d{1,3}(?:[\s\u00a0]\d{3})*)",
        ]
        ...
        # Address
        addr_tag = soup.find(class_=re.compile(r"address|adress|gata", re.IGNORECASE))
        if addr_tag:
            data["address"] = addr_tag.get_text(strip=True)
        else:
            addr_match = re.search(r"([A-ZÄÖÅ][a-zäöå]+(?:gatan|vägen|gränd)\s*\d*)", full_text)
            if addr_match:
        ...
        # Queue days required (specific to this site)
        queue_match = re.search(r"(\d+)\s*(?:ködag|ködagar|queue days)", full_text, re.IGNORECASE)
        if queue_match:
            data["queue_days_required"] = int(queue_match.group(1))
        ...
        data["agency"] = "Bostadsförmedlingen Stockholm"
        return data
