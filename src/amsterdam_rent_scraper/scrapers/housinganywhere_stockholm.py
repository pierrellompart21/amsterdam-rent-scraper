"""HousingAnywhere.com scraper for Stockholm - International rental platform."""
...
class HousinganywhereStockholmScraper(PlaywrightBaseScraper):
    """Scraper for housinganywhere.com Stockholm listings.

    HousingAnywhere is an international platform for mid-term rentals,
    popular with students, expats, and professionals.

    URL patterns:
    - Search: https://housinganywhere.com/s/Stockholm--Sweden
    - Listing: /Stockholm--Sweden/{property-slug}
    """

    site_name = "housinganywhere_stockholm"
    ...
    def get_search_url(self, page_num: int = 1) -> str:
        ...
        base = f"{self.base_url}/s/Stockholm--Sweden"
        params = {}
        # HousingAnywhere uses EUR
        if self.min_price:
            params["priceMin"] = int(self.min_price / 11.5)  # SEK to EUR
        if self.max_price:
            params["priceMax"] = int(self.max_price / 11.5)
        if page_num > 1:
    ...
    def get_listing_urls(self, page: Page) -> list[str]:
        ...
        while current_page <= max_pages:
            try:
                ...
                # Find listing links - pattern varies
                listing_links = soup.find_all("a", href=re.compile(r"/Stockholm--Sweden/[^/]+"))
                ...
                for link in listing_links:
                    href = link.get("href", "")
                    # Extract slug as ID
                    id_match = re.search(r"/Stockholm--Sweden/([^/?]+)", href)
                    if id_match:
                        listing_id = id_match.group(1)
                        if listing_id not in seen_ids and listing_id != "s":
    ...
    def parse_listing_page(self, html: str, url: str) -> dict:
        ...
        # Price - HousingAnywhere shows EUR primarily
        ...
        if "price_sek" not in data:
            sek_match = re.search(r"(\d{1,3}(?:[\s,]\d{3})*)\s*(?:SEK|kr)", full_text, re.IGNORECASE)
            if sek_match:
        ...
        # Surface area
        surface_match = re.search(r"(\d+(?:[.,]\d+)?)\s*(?:m²|m2|sqm)", full_text, re.IGNORECASE)
        if surface_match:
            try:
        ...
        # Bedrooms
        bed_match = re.search(r"(\d+)\s*(?:bedroom|bed)\b", full_text, re.IGNORECASE)
        if bed_match:
            try:
        ...
        # Property type from page
        if re.search(r"\bstudio\b", full_text, re.IGNORECASE):
            data["property_type"] = "Studio"
        elif re.search(r"\broom\b", full_text, re.IGNORECASE):
        ...
        # Furnished - HousingAnywhere listings are typically furnished
        if re.search(r"\bfurnished\b", full_text, re.IGNORECASE):
            data["furnished"] = "Furnished"
        elif re.search(r"\bunfurnished\b", full_text, re.IGNORECASE):
            data["furnished"] = "Unfurnished"
        else:
            data["furnished"] = "Furnished"  # Default for this platform
        ...
        if re.search(r"\bparking\b", full_text, re.IGNORECASE):
        ...
        if re.search(r"available\s*now|immediately", full_text, re.IGNORECASE):
            ...
            date_match = re.search(r"(\d{1,2}\s+\w+\s+\d{4}|\d{4}-\d{2}-\d{2})", full_text)
            if date_match:
