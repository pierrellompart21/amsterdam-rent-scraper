"""Bostadsportalen.se scraper - Swedish rental aggregator (JavaScript rendered)."""
...
class BostadsportalenScraper(PlaywrightBaseScraper):
    """Scraper for bostadsportalen.se Swedish rental listings.

    Bostadsportalen is a rental listings aggregator for Stockholm.
    The site requires JavaScript rendering.

    URL patterns:
    - Search: https://www.bostadsportalen.se/hyresratter/stockholm
    - Listing: https://www.bostadsportalen.se/hyresratt/{id}
    """

    site_name = "bostadsportalen"
    ...
    def get_search_url(self) -> str:
        ...
        return f"{self.base_url}/hyresratter/stockholm"
    ...
    def get_listing_urls(self, page: Page) -> list[str]:
        ...
        try:
            ...
            # TODO: Adjust selector based on actual Bostadsportalen HTML structure
            for link in soup.find_all("a", href=True):
                href = link.get("href", "")
                if "/hyresratt/" in href:
                    listing_match = re.search(r"/hyresratt/([^/]+)", href)
                    if listing_match:
    ...
    def parse_listing_page(self, html: str, url: str) -> dict:
        """Parse a Bostadsportalen listing page and extract data."""
        soup = BeautifulSoup(html, "lxml")
        data = {}
