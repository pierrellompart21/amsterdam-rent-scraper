"""BostadDirekt.com scraper - Swedish direct rental listings (JavaScript rendered)."""
...
class BostadDirektScraper(PlaywrightBaseScraper):
    """Scraper for bostaddirekt.com Swedish rental listings.

    BostadDirekt offers direct rental listings from both private landlords and agencies.
    The site is JavaScript-heavy. Prices are in SEK.

    URL pattern: /hyra-lagenhet/[area]/[listing-id]
    """

    site_name = "bostad_direkt"
    ...
    def get_search_url(self) -> str:
        ...
        base_search = f"{self.base_url}/hyra-lagenhet/stockholm"
        params = []
        if self.min_price:
            params.append(f"rent_min={self.min_price}")
        if self.max_price:
            params.append(f"rent_max={self.max_price}")
        if params:
    ...
    def get_listing_urls(self, page: Page) -> list[str]:
        ...
        try:
            ...
            for _ in range(10):
                ...
                # TODO: Update selector based on actual BostadDirekt HTML structure
                links = soup.select('a[href*="/objekt/"], a[href*="/lagenhet/"]')
            ...
            # Find listing links
            # TODO: Update selector based on actual BostadDirekt HTML structure
            for link in soup.select('a[href*="/objekt/"], a[href*="/lagenhet/"]'):
                href = link.get("href", "")
    ...
    def parse_listing_page(self, html: str, url: str) -> dict:
        """Parse a BostadDirekt listing page and extract data."""
        soup = BeautifulSoup(html, "lxml")
        data = {}
