...
class HemaviScraper(PlaywrightBaseScraper):
    ...
    Hemavi is a newer rental marketplace with a modern interface.
    The site is JavaScript-heavy. Prices are in SEK.

    URL pattern: /sv/hyra/[area]/[listing-id]
    """
    ...
    def get_search_url(self) -> str:
        ...
        base_search = f"{self.base_url}/sv/hyra/stockholm"
        params = []
        if self.min_price:
    ...
    def get_listing_urls(self, page: Page) -> list[str]:
        ...
        try:
            ...
            for _ in range(10):
                ...
                # TODO: Update selector based on actual Hemavi HTML structure
                links = soup.select('a[href*="/hyra/"], a[href*="/objekt/"]')
            ...
            # Find listing links
            # TODO: Update selector based on actual Hemavi HTML structure
            for link in soup.select('a[href*="/hyra/"], a[href*="/objekt/"]'):
                href = link.get("href", "")
                # Exclude search/category pages
                if ("/hyra/" in href or "/objekt/" in href) and href.count("/") > 3:
